        ...
        for i in range(len(data_months)):
            for j in range(len(data_years)):
                inflation_data.append({
                    'Tahun': data_years[j], 'Bulan': data_months[i],
...
def scrape_bi_rate():
    try:
        ...
        for kode, value in datacontent.items():
            ...
            try:
                if len(timecode) == 3:
                    bulan = int(timecode[2]); tahun = 2000 + int(timecode[:2])
                elif len(timecode) == 4:
                    bulan = int(timecode[2:]); tahun = 2000 + int(timecode[:2])
                else: continue
...
def run_scraping(table_name, df_actual):
    ...
    for p in process_periods:
        ...
        updates['Effective Working Days'] = get_effective_working_days(p.year, p.month)
...
def show():
    ...
    st.subheader("📋 Data Aktual SBB")
    st.dataframe(df, use_container_width=True)

    with st.expander("🔄 Update Data Otomatis (Scraping)", expanded=False):
        st.caption("Inflasi & BI Rate dari API BPS, EWD dari holidays, PDB Konstruksi dari tabel forecast.")
        if st.button("Ambil Data dari API", type="primary", key="scrape_sbb"):
            with st.spinner("Mengambil dan memproses data..."):
                count = run_scraping("sbb_actual", df)
    ...
    with col1:
        with st.expander("➕ Input Data Baru", expanded=True):
            ...
            periode = st.date_input("Periode", value=default_p, format="YYYY-MM-DD", key="s_new_p")
            inflasi = st.number_input("Inflasi", value=0.0, format="%.6f", key="s_new_inf")
            bi_rate = st.number_input("BI Rate", value=0.0, format="%.6f", key="s_new_bi")
            pdb = st.number_input("PDB Konstruksi", value=0.0, format="%.2f", key="s_new_pdb")
            ewd = st.number_input("EWD", min_value=0, max_value=31,
                                  value=int(get_effective_working_days(default_p.year, default_p.month)), key="s_new_ewd")
            volume = st.number_input("Volume Aktual (m³)", value=0.0, format="%.2f", key="s_new_vol")

            if st.button("Simpan", type="primary", key="s_save"):
                database.update_single_row("sbb_actual", periode.strftime('%Y-%m-%d'), {
    ...
    with col2:
        with st.expander("✏️ Edit Data", expanded=True):
            if not df.empty:
                ...
                sel = st.selectbox("Pilih Periode", p_list, index=len(p_list)-1, key="s_edit_sel")
                p = pd.to_datetime(sel)
                row = df.loc[p]
                
                # Gunakan dynamic key f"{key}_{sel}" agar widget refresh saat periode ganti
                e_inf = st.number_input("Inflasi", value=float(row.get('Inflasi', 0) or 0), format="%.6f", key=f"s_e_inf_{sel}")
                e_bi = st.number_input("BI Rate", value=float(row.get('BI Rate', 0) or 0), format="%.6f", key=f"s_e_bi_{sel}")
                e_pdb = st.number_input("PDB Konstruksi", value=float(row.get('PDB Konstruksi', 0) or 0), format="%.2f", key=f"s_e_pdb_{sel}")
                raw_ewd = row.get('Effective Working Days', 0)
                val_ewd = int(raw_ewd) if pd.notna(raw_ewd) else 0
                e_ewd = st.number_input("EWD", min_value=0, max_value=31, value=val_ewd, key=f"s_e_ewd_{sel}")
                e_vol = st.number_input("Volume (m³)", value=float(row.get('Volume', 0) or 0), format="%.2f", key=f"s_e_vol_{sel}")
                
                if st.button("Perbarui", type="primary", key="s_update"):
                    database.update_single_row("sbb_actual", sel, {
    ...
    with col3:
        with st.expander("🗑️ Hapus Data", expanded=True):
            if not df.empty:
                h_p = st.selectbox("Pilih Periode", df.index.strftime("%Y-%m-%d").tolist(), key="s_del_sel")
                confirm = st.checkbox("Konfirmasi hapus", key="s_confirm")
                if st.button("Hapus", type="primary", key="s_del"):
                    if confirm:
                        database.delete_row("sbb_actual", h_p)
    ...
    edited_exog = st.data_editor(exog_2026, use_container_width=True, key="s_exog_editor")
    if st.button("Simpan Perubahan Eksogen 2026", key="s_save_exog"):
