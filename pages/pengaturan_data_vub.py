...
def show():
    ...
    with st.expander("🔄 Update Data Otomatis (Scraping)", expanded=False):
        ...
        if st.button("Ambil Data dari API", type="primary", key="scrape_vub"):
            with st.spinner("Mengambil dan memproses data..."):
                count = run_scraping("vub_actual", df)
    ...
    with col1:
        with st.expander("➕ Input Data Baru", expanded=True):
            ...
            periode = st.date_input("Periode", value=default_p, format="YYYY-MM-DD", key="v_new_p")
            inflasi = st.number_input("Inflasi", value=0.0, format="%.6f", key="v_new_inf")
            bi_rate = st.number_input("BI Rate", value=0.0, format="%.6f", key="v_new_bi")
            pdb = st.number_input("PDB Konstruksi", value=0.0, format="%.2f", key="v_new_pdb")
            ewd = st.number_input("EWD", min_value=0, max_value=31,
                                  value=int(get_effective_working_days(default_p.year, default_p.month)), key="v_new_ewd")
            volume = st.number_input("Volume Aktual (m³)", value=0.0, format="%.2f", key="v_new_vol")

            if st.button("Simpan", type="primary", key="v_save"):
                database.update_single_row("vub_actual", periode.strftime('%Y-%m-%d'), {
    ...
    with col2:
        with st.expander("✏️ Edit Data", expanded=True):
            if not df.empty:
                ...
                sel = st.selectbox("Pilih Periode", p_list, index=len(p_list)-1, key="v_edit_sel")
                p = pd.to_datetime(sel)
                row = df.loc[p]
                
                # Gunakan dynamic key f"{key}_{sel}" agar widget refresh saat periode ganti
                e_inf = st.number_input("Inflasi", value=float(row.get('Inflasi', 0) or 0), format="%.6f", key=f"v_e_inf_{sel}")
                e_bi = st.number_input("BI Rate", value=float(row.get('BI Rate', 0) or 0), format="%.6f", key=f"v_e_bi_{sel}")
                e_pdb = st.number_input("PDB Konstruksi", value=float(row.get('PDB Konstruksi', 0) or 0), format="%.2f", key=f"v_e_pdb_{sel}")
                raw_ewd = row.get('Effective Working Days', 0)
                val_ewd = int(raw_ewd) if pd.notna(raw_ewd) else 0
                e_ewd = st.number_input("EWD", min_value=0, max_value=31, value=val_ewd, key=f"v_e_ewd_{sel}")
                e_vol = st.number_input("Volume (m³)", value=float(row.get('Volume', 0) or 0), format="%.2f", key=f"v_e_vol_{sel}")
                
                if st.button("Perbarui", type="primary", key="v_update"):
                    database.update_single_row("vub_actual", sel, {
    ...
    with col3:
        with st.expander("🗑️ Hapus Data", expanded=True):
            if not df.empty:
                h_p = st.selectbox("Pilih Periode", df.index.strftime("%Y-%m-%d").tolist(), key="v_del_sel")
                confirm = st.checkbox("Konfirmasi hapus", key="v_confirm")
                if st.button("Hapus", type="primary", key="v_del"):
                    if confirm:
                        database.delete_row("vub_actual", h_p)
    ...
    edited_exog = st.data_editor(exog_2026, use_container_width=True, key="v_exog_editor")
    if st.button("Simpan Perubahan Eksogen 2026", key="v_save_exog"):
