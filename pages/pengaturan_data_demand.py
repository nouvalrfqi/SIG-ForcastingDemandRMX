...
def scrape_apbn_infra():
    ...
    df_existing = st.session_state.df_demand
    ...
    for idx, row in df_existing.iterrows():
        ...
        total_demand = df_existing[df_existing.index.year == comparison_year]['Demand'].sum()
        ratio = demand / total_demand if total_demand > 0 else 0
...
def process_all_columns(df_existing, scraped_data_dict, sheet_updater, update_forecasting, start_year=2021):
    ...
    if update_forecasting:
        sheet_updater(forecast_assumptions, sheet_name="Forecasting Demand")

    return updated_actuals

def data_scraping(df, forecasting_assumptions, sheet_updater, update_forecasting, start_year=2021):
    ...
    updated_actuals['Demand'] = updated_actuals['Demand'].fillna(0)
    ...
    sheet_updater(updated_actuals, sheet_name="Demand")
    return updated_actuals

st.title("⚙️ Pengaturan Data Demand")

if "df_demand" not in st.session_state:
    st.session_state.df_demand = reload_df(conn, "Demand")

if "df_forecasting_assumptions" not in st.session_state:
    st.session_state.df_forecasting_assumptions = reload_df(conn, "Forecasting Demand")

df = st.session_state.df_demand 
...
with st.expander("🔄 Update Data Otomatis", expanded=True):
    ...
    if st.button("Ambil Data dari API", type="primary"):
        with st.spinner("Mengambil dan memproses data..."):
            try:
                update_actuals = data_scraping(df, forecasting_assumptions, update_df_to_gsheet, update_forecasting, 2021)
                st.session_state.df_demand = update_actuals
...
with col1:
    with st.expander("➕ Input Data Baru", expanded=True):
        ...
        ewd = st.number_input("Hari Kerja Efektif", min_value=1, max_value=31,
                                ...
                                key="input_ewd")
        demand = st.number_input("Demand Aktual", value=0.0, format="%.2f", key="input_demand")
        ...
        if submit:
            if periode in df.index:
                ...
                st.session_state.df_demand = df
                update_df_to_gsheet(st.session_state.df_demand)
...
with col2:
    with st.expander("✏️ Edit Data", expanded=True):
        ...
        if periode_edit:
            ...
            demand_val = safe_value(df.loc[p, "Demand"], float, 0.0)
